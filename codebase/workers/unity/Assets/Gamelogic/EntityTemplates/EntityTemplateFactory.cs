...
public class EntityTemplateFactory : MonoBehaviour
{
    ...
    public static Entity CreateDroneTemplate(Improbable.Coordinates spawnPoint, Vector3f initialTarget, float droneSpeed, float hitRadius, bool snapshot = false)
    {
        var droneTemplate = EntityBuilder.Begin()
            ...
            .SetReadAcl(CommonRequirementSets.PhysicsOrVisual)
            .AddComponent(new Rotation.Data(Quaternion.identity.ToNativeQuaternion()), CommonRequirementSets.PhysicsOnly)
            .AddComponent(new DroneData.Data(initialTarget, Mathf.Max(SimulationSettings.MaxDroneSpeed, droneSpeed), hitRadius, TargetPending.REQUEST, snapshot), CommonRequirementSets.PhysicsOnly)
            .Build();
