...
\newpage
\section{TODO: Implementation of Core Entities}
...
To complement the C\# class that SpatialOS generates from this, we have implemented a static class containing methods that are necessary for integrating No Fly Zones with the Global Layer and Bitmap components. These are checks to see if:
\begin{itemize}
...
The benefit of this implementation is that only relevant data is stored in the map, with failure to find a cell in the map implying that the corresponding area of the world is not in or near a No Fly Zone. Since every controller will have a Bitmap component, this design improves memory efficiency and provides more options to the controller. It could simply reduce its memory footprint, deal with larger areas, or reduce the cell size to represent the world witha a higher precision.
...
In Figure \ref{fig:basic_controller_test}, drones are spawned either side of the No Fly Zone. We check whether the controller returns waypoints in the correct order and if the flight path avoids the obstacle.
...
\section{No Fly Zone Static Class}
\begin{sexylisting}[colback=white]{NoFlyZone.cs Static Class Overview}
