...
\section{TODO: Generating Snapshots}
...
Figure \ref{fig:unity_londonsnapshotsmallexample} shows how the London scene looks in Unity when visually editing a snapshot. The leftmost column is known as the \textit{Hierarchy}, a list of all the objects currently in the scene. To the side of this is the \textit{Inspector} that shows all the Unity components on a selected prefab, in this case a Drone.
...
The world view in the middle is for visual aid when placing entities in the scene, and the column on the far right is a menu for SpatialOS build operations.
% This menu is particularly useful as being able to quickly rebuild the project within the Unity Editor greatly reduces iteration times.
