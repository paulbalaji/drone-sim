...
%**********************************************%
\newpage
\section{REVISIT: Integrating Delivery Economics}
...
\newpage
We know that the power rating of a moving drone is 8355.1W, but we must compute the power of a drone hovering ourselves using the current and voltage readings.
$$energy = power * time$$
...
Understandably, nothing will ever work perfectly forever. To mitigate bugs in the simulation, we periodically ``prune'' drones that have been unresponsive for too long. We therefore consider the penalties applied for actions the drone has failed to complete at the time of pruning.

\newpage
We define four main infractions.
\begin{enumerate}
...
\subsection{Delivering a Milkshake}
...
\begin{figure}[!hbpt]
  \center
  \includegraphics[width=\linewidth]{img/drone_delivery_example_cost.png}
  \caption{Measuring the total distance of the delivery round trip.}
  \label{fig:drone_delivery_example_cost}
\end{figure}

\newpage
As per Table \ref{fig:package_revenue_distribution}, the revenue generated for standard delivery of an 800g milkshake is £1.84 for a operating costs of just 5.53p to the delivery network operator. As this is a mostly-autonomous system, the lion's share of generated revenue is pure profit. For comparison, a large portion of Deliveroo's standard delivery fee £2.50 may be used to pay the delivery person. Perhaps these monetary, time and efficiency savings fuel future consumer demand for increased drone adoption.

%**********************************************%
\newpage
\section{REVISIT: Time-Value of Delivery}
...
An array of 10 booleans would take 10 bits, but an array of 10 integers representing the value at each step would be 40 bytes, 32 times the number of bits. Considering a TVF will be sent with every delivery request and each controller maintains a queue of requests, reducing the memory footprint of a TVF may allow the simulation to scale better down the line.
...
To integrate TVFs with our simulation, we update the Order Generator to support two models. For every delivery request generated, the Order Generator randomly chooses between \textit{The Halvening} or \textit{Stepwise Decrease} functions. Although we could simply enumerate these two models and calculate the delivery fee using pre-defined value functions, our chosen implementation opens up the option to experiment with more complex TVFs in the future.

\subsubsection{The Halvening}
\textit{The Halvening} is a formalisation of the price-halving model we had already implemented before we introduced Time-Value Functions.

\begin{figure}[!hbpt]
  \center
  \includegraphics[width=0.65\linewidth]{img/tvfs_halvening.png}
  \caption{Plotting \textit{The Halvening}.}
  \label{fig:tvfs_halvening}
\end{figure}

\subsubsection{Stepwise Decrease}
Customers may favour this more as it enforces a fee decrease every 6 minutes, which means Controllers have even more incentive to deliver packages as quick as possible.

\begin{figure}[!hbpt]
  \center
  \includegraphics[width=0.65\linewidth]{img/tvfs_decrease.png}
