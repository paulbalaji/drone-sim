...
No matter how much is done to make the logic of the simulation as efficient and scalable as possible, there always comes a time where we must look at modifying the SpatialOS worker configurations in order to make better use of the underlying platform.
...
In fact, the only intentional cross-worker communication is from the OrderGenerator to a target Controller when sending a new \texttt{DeliveryRequest} at its defined rate. Essentially, by distributing the raw workload in a similar way to the distribution of delivery requests across Controllers, we obtain an efficient, parallel, distributed simulation.

\subsection{Chunk Size}
% size 50
% link to total number of chunks in the world then

\subsection{Entity Interest Range}
...
% 200 --> 25
% what information about its surroundings does a drone need to determine for itself?
% only nearby drones, so we set range to 25
% relate to max distance that a drone needs to be able to sense around it
