...
To simplify the problem and reduce the computation complexity of the scheduler, we opt out of modelling duration as a distribution. Instead, we simply treat the duration as a fixed value derived from the distance between the controller and the delivery destination.
