...
% \HRule \\[0.4cm]
% % { \huge \bfseries Drone Delivery Simulation\\using SpatialOS}\\[0.4cm] % Title
% % { \huge \bfseries Make Drones Not Do A Crash:\\A SpatialOS Story}\\[0.4cm] % Title
% { \huge \bfseries Simulating Drone Delivery Networks\protect\\using SpatialOS}\\[0.4cm] % Title of your
% \HRule \\[0.4cm]
...
\begin{abstract}
Autonomous vehicles are an area of Engineering where there is intense research taking place in both the private sector and academia. Most news articles tend to focus on autonomous cars and trucks, but drones are often over-looked as a novelty gadget for filming holidays with.\\

This project investigates the use of drones as a medium of transporting goods in a distributed drone-based delivery network. We port existing routing solutions to SpatialOS, a distributed simulation platform, and update the interaction model to improve autonomy of the system.\\

We then introduce a novel time-value based pricing model to go hand-in-hand with a new time-value based scheduling algorithm, originally utilised for big data applications. Lastly, we evaluate the effectiveness of this new algorithm against both a standard FIFO queue and shortest-job first ordering.
\end{abstract}

\renewcommand{\abstractname}{\large Acknowledgements}
\begin{abstract}
A project of this size always takes a great mental, physical and emotional toll on oneself, and I could not have completed this project without the love and support of a few people in my life:
\begin{itemize}
  \item My supervisor, Will, for being my programming tutor in first year, allowing me to be his UTA in third year, and letting me continue to work on drones for my project.
  \item The people of Imperial. It's been a whirlwind of a time, and I will miss you all.
  \item The extended El Sesh family. This final year would not have been as amazing as it was without every single one of you.
  \item My report-writing library squad: Elliot \& Michael.
  \item My miscellaneous friends. Jack, for the countless hours of life advice. Aaliya, for just being the weird \& wonderful psycho that you are.
  \item My surrogate web apps group: Subhash, Inara, Jayati \& Benito. You have all been so supportive in the two most stressful weeks of my life and I can never repay you for welcoming me with open arms.
  \item My old friends. James, James, Nisha, Paul, Ale \& Sam. Thank you for sticking by me.
  \item My oldest friend, Tarun. Thank you for being the person that you are, a real stand-up guy who somehow manages to put up with my stress.
  \item Last and certainly not the least, my family. Mum, thank you for taking care of me, getting me through the 11+, and always pushing me to work as hard as I can. Dad, thank you for inspiring me to make the most of the chances life gives me. Jennifer, thank you for always being the most annoyingly adorable bundle of joy in my life.
\end{itemize}

I am who I am because of all of you. Thank you everyone.
\end{abstract}
...
In this report, we leverage the power of SpatialOS to produce novel simulations of a drone delivery network. The simulation will build upon existing routing solutions\cite{Balaji2017} and would aim to schedule deliveries based on a time-value function borrowed from the big data use case. Furthermore, we wish to show how the profitability of a drone delivery network with this form of advanced scheduling might compare to delivery networks that utilise a more basic scheduling algorithm.

\clearpage
\section{Contributions}
As a result of our efforts in this project, we present 7 key contributions.
\begin{enumerate}
  \item SpatialOS port of AATC (Chapter 3)
  \begin{itemize}
    \item We take our knowledge of previously developed drone routing systems and port them to SpatialOS as the basis of our delivery network architecture.
  \end{itemize}
  \item Basic Delivery Network Architecture (Chapter 5)
  \begin{itemize}
    \item We define a basic set of components and interactions that would allow for rudimentary simulations of a delivery network to be done.
  \end{itemize}
  \item Snapshot Generation Tooling (Chapters 6, 10)
  \begin{itemize}
    \item We implemented a set of tools to aid snapshot generation, in order to generate more accurate snapshots of London.
  \end{itemize}
  \item Drone Energy Cost Simulation (Chapter 7)
  \begin{itemize}
    \item We investigate the cost of running an unmanned aerial delivery vehicle, and model the energy consumption (and subsequent electricity bill) of drones in our simulation.
  \end{itemize}
  \item Time-Value of Delivery (Chapter 8)
  \begin{itemize}
    \item We adapt the idea that something has higher value the sooner it is used from big data for our delivery use case.
  \end{itemize}
  \item LLV Scheduling (Chapter 9)
  \begin{itemize}
    \item We adapt a big data scheduling tool to work in the drone delivery use case.
  \end{itemize}
  \item Scaling Strategies for SpatialOS (Chapter 10, 11)
  \begin{itemize}
    \item We describe a few methods that may enable our simulations to scale in size and complexity.
  \end{itemize}
\end{enumerate}
...
\begin{figure}[!hbpt]
  \center
  \includegraphics[width=\linewidth]{img/may_03_123.png}
...
% \section{Problems}
% \subsubsection{Worker Genocide}
% For some unexplained reason, we no
%
% \subsection{OrderGenerator Downtime}
% \subsection{Max Unacked Pings Rate}
%
% Despite encountering several problems in our efforts to get our code working flawlessly with the large London snapshot, the fact that our basic delivery network model scales for the most part is a positive sign. There are room for improvements throughout all parts of this project, but most evidently in the challenge of scaling the simulation.\\
%
% Remember that we originally designed the larger snapshot to put more load on each Controller, and that the LLV algorithm works best when there is enough load to keep it busy. At least per our hypothesis, the LLV policy should be the optimal scheduling strategy for this large scale simulating.
...
% controller drones queue deliveries fullTrips revenue costs penalties avgDelTime avgWaitTime fDel fRet fLaunch collisions unknown rejecValue avgRejecValue potentialLost avgPotentialLost
...
\clearpage
\subsection{Summary}
From the results in Figure \ref{fig:small_deltime}, it seems that schedulers do not have much influence over the time a drone spends out for delivery when there is a low load. In fact, it seems the choice of scheduler does not affect the results by any significantly amount when the load is low such as in C3. This is to be expected because, like we have stated already, there is no contention for resources in C3 and thus no need or discernable gain to be had if one was to order requests by priority.\\

However for C2 we can clearly see the effect that SJF has, as its plot on Figure \ref{fig:small_deltime} is generally lower than the other schedulers. This is also to be expected as the whole \textit{shtick} of SJF is to prioritise and execute the least time consuming jobs available. In our drone case, the ones which spend the least time out for delivery.\\

Nonetheless, we believe the most interesting story being told by Figure \ref{fig:small_deltime} is the intertwining of LLV and FCFS. It shows that although they spend a similar time out on average, even completing a very similar volume of deliveries (Figure \ref{fig:small_deliveries}) , the LLV scheduler's intent to maximimise the average profitability of each delivery proves fruitful overall (Figure \ref{fig:small_profit}).\\

When there is no contention for resources one could say SJF might be slightly more profitable than the other two (Figure \ref{fig:small_profit}), but we seek to find the scheduler that performs best under heavier loads, in order to equip ourselves for a future where drones are the primary medium of delivering goods.\\

Consequently, the Least Lost Value policy is our proposed scheduling approach for the drone delivery use case.

%**********************************************%
\chapter{Conclusions}
We began by re-introducing the concept of a two-layered drone routing system as described by AATC. After updating the old interaction model to obtain greater autonomy, we brought across both AATC layers to SpatialOS and integrated it with a very primitive drone spawning entity.\\

We then outlined the basic system architecture of a simple drone delivery network, allowing centralised work from AATC to be applied to the massively distributed problem of distributed delivery networks. To test this architecture, we created some tooling, processes and guidelines for building snapshots of drone delivery network.\\

After testing this new architecture, we introduced a new pricing model based on time-value functions that would reward the drone operator more for quicker delivieres and provide greater fee reductions to the customer for slower deliveries. Subsequently we looked into time-value based scheduling algorithms and adopted one such method, \textit{Least Lost Value}, to work in the drone delivery case despite originally being a tool for big data.\\

We also investigated ways to better utilise the SpatiaOS platform in order to scale up both the size and complexity of our current simulations.\\

Lastly, we evaluated the results of our simulations to find that the LLV scheduler is best for the workloads that we are expecting.

%**********************************************%
\chapter{Future Work}
Our initial aim for the project was to conceptualise, build and simulate a drone delivery network using cutting-edge technology. To this end, we believe that our project exceeds these goals because we have also proposed methods of distributing, optimising and scaling up the system.\\

Despite this, we believe that there are still plenty of ways in which our work could be extended to better simulate drone delivery networks.

\subsubsection{Splitwise Scheduling}
This would be a new type of scheduler that is half Least Lost Value (LLV), half Shortest Job First (SJF). Perhaps this could be achieved by running two schedulers on a Controller at once and splitting the number of drones that each scheduler is allowed to allocate jobs for. Though we are not sure what effect performing this split would have, it may be a good idea to combine the two as SJF managed to complete a higher number of deliveries overall.

\subsubsection{Inter-Controller Communication}
The main inspiration for this feature is to allow delivery requests to flow between Controllers, instead of only being sent out by the Order Generator. This would be beneficial for the delivery network because nearby Controllers who have the drone capacity to make a delivery can pick up this request and act upon it.\\

Although it would result in slightly higher energy costs due to the destination being further away than that Controller's typical delivery request, taking on other Controllers' tasks when idle would mean that work is better distributed across the network.

\subsubsection{Dynamic No Fly Zones}
At the moment, the pathfinding layer has too easy of a time because once it has been initialised it only ever returns paths between points. To make this more interesting, we could introduce the concept of a dynamic NFZ, where the zone is either moving, constantly changing shape, only active at certain times, or all of the above.\\

Such a zone might be useful in the event of a visit from a foreign diplomat, when the authorities wish to close up parts of the sky that are too close to said diplomat's moving aircraft.

\subsubsection{Interactive UnityClient}
One of the features of AATC that we have not been able to match within the time bounds of this project is the visualiser tool that showed the paths that each drone took. Such a tool might be unfeasible to mimic since we intend to run our SpatialOS simulations at a much higher scale than AATC, but we propose a similar tool that allows a layman to interact with the settings, toggle No Fly Zones, and perhaps dynamically add custom zones of their own.
...
% better load balancing (is this possible without manually defining controller positions)
