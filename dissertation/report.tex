...
That is a 5.53p cost to the delivery network operator for a mostly-autonomous system. For comparison Deliveroo's standard delivery fee is currently £2.50, a large portion of which may be used to pay the delivery person. Perhaps these monetary, time and efficiency savings fuel future consumer demand for increased drone adoption.

\newpage
\subsubsection{Penalties}
Understandably, nothing will ever work perfectly forever. Therefore we must consider what the cost of certain failed action are. We define four main infractions.
\begin{enumerate}
...
If a drone is launched but still fails to deliver a package for whatever reason, we compensate the consumer as before and also calculate the cost of sending a truck to manually retrieve the drone that was sent out. When a drone fails for some reason \textit{after} making a delivery there is no reason to compensate the customer, but we still have to calculate the cost of retrieving the drone.\\
