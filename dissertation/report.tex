...
Despite encountering several problems in our efforts to get our code working flawlessly with the large London snapshot, the fact that our basic delivery network model scales for the most part is a major achievement.\\

Preliminary results for simulations run with the large London snapshot indicate that the behaviour we observed by all three schedulers at small scale are also present here. This is fantastic insight because we designed the larger snapshot to put more load on each Controller, and the LLV algorithm works best when there is sufficient load to keep it busy.\\

Connecting the dots, it means that both the Least Lost Value algorithm and corresponding concept of Time-Value Functions have been vindicated for our use case of a distributed drone delivery network.

% despite problems, we were able to get enough data from the small snapshot that it didn't matter too much how it performed at scale
...
Our aim for the project was to X --> did we do this or not?

Summary of work:

In this project we have A

As part of the process, we did B

From B, we expanded to C which enabled us to do D



%**********************************************%
\chapter{Future Extensions}
Due to the ambitious nature of the project, there are plenty of ways in which our work could be extended to provide a tangible improvement to
