...
\section{TODO: Achieving London-Scale Simulation}
...
To generate our own Voronoi diagram of the world, we assign each controller a colour, traverse the map and assign a colour to each point based on which controller was closer. After looping through the map, we are given a nice visual representation of the area split. One observes in Figure \ref{fig:smol} that the two shaded regions of the world are fairly even in size, a positive sign that delivery requests are likely to be balanced well between controllers.

\begin{figure}[!hbpt]
  \center
  \includegraphics[width=\linewidth]{img/smol.png}
  \caption{Voronoi diagram of the London snapshot.}
  \label{fig:smol}
\end{figure}

When inspecting a live simulation \ref{fig:gdoc2}, one can clearly see the worker boundary split through the middle. As mentioned before, SpatialOS constantly tweaks the load balancer so the boundaries may move around slightly as each region's load rises and falls with the flow of incoming orders.

\begin{figure}[!hbpt]
  \center
  \includegraphics[width=\linewidth]{img/gdoc2.png}
...
\subsection{Summary}
With a basic simulation now implemented and handling the logistical operations of a drone delivery network, we now look at how to extend our work to factor in the economic side.
